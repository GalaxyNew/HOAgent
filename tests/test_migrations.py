...
def test_up_down_and_integrity(tmp_path):
    db=tmp_path/'ops.db'; assert migrate_up(db) == '004_governance_knowledge_fts'
    conn=connect(db); assert conn.execute('PRAGMA integrity_check').fetchone()[0] == 'ok'; assert conn.execute('PRAGMA foreign_key_check').fetchall() == []; assert conn.execute('PRAGMA journal_mode').fetchone()[0].lower() == 'wal'; conn.close()
    assert migrate_down(db) == '004_governance_knowledge_fts'; assert migrate_down(db) == '003_temporal_update_guard'; assert migrate_down(db) == '002_projection_pipeline'; assert migrate_down(db) == '001_stage1'
    conn=sqlite3.connect(db); assert conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='task'").fetchone() is None

def test_update_overlap_trigger_rejects_overlap(tmp_path):
    db=tmp_path/'ops.db'; migrate_up(db); conn=connect(db)
    ...
    with pytest.raises(sqlite3.IntegrityError, match='overlaps'): conn.execute("UPDATE business_relationship SET effective_from='2026-01-15' WHERE relationship_id='r2'")

def test_knowledge_fts_soft_delete_and_governance_columns(tmp_path):
    db=tmp_path/'ops.db'; migrate_up(db); conn=connect(db)
    assert 'responsibility_chain' in {r[1] for r in conn.execute('PRAGMA table_info(business_entity)')}; assert {'source_type','responsibility_chain'} <= {r[1] for r in conn.execute('PRAGMA table_info(business_relationship)')}
